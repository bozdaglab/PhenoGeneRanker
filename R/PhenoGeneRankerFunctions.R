...
#'   value of 100.
#'   
#' @param walkMatrix This is the walk matrix that gets generated by the method
#'   CreateWalkMatrix.
...
#'   corresponds to the order of the files in your input file. The weights must
#'   sum up to the same number of gene files you added. If you want to have each
...
#'   input files that were added in CreateWalkMatrix. Each index of the vector
#'   corresponds to the order of the files in your input file. The weights must
#'   sum up to the same number of phenotype files you added. If you want to have
#'   each file equally weighted, you do not need to input this vector to the
#'   parameters of this method.
#'
...
# gene and phenotype data files. Has to be a '.txt' file. This text file is made
# from two columns that are tab-separated. the first row needs the two columns to
# say type and file_name. Every row after that contains what kind of data each
# file would contain. These can be either gene, phenotype, or bipartite files.
# Under file_name, you will input the name of the file along with the .txt
# extension. Inside each file_name file, there is a from, to, and weight column
# which are all tab-separated. For gene and phenotype layer files, the from and
# to columns will have genes and phenotypes, the order doesn’t matter. The weight
# column will have the weight of the relationship of the genes and phenotypes,
# for unweighted network all weights must have a value of 1. For a bipartite
# file, the from column must have genes, the to column must have phenotypes, the
...
#' @param zeta This is the probability of jumping between phenotype layers. It
#'   has a range of 0 to 1. It has a default value of 0.5.
...
#'   by name, list of phenotypes sorted by name, the degree of all genes in the
#'   multiplex heterogeneous network, the degree of all phenotypes in the
#'   network, the number of gene layer files, the number of phenotype layer
#'   files, the number of gene pool nodes, and the number of phenotype nodes.
#'   
#' @export
#'
#' @examples
